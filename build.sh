...
setup_toolchains() {
    ...
    case "$CLANG_VERSION" in
        ...
        "aosp-clang-20")
            ...
            ;;
        *)
            pr_warn "Unknown Clang version: $CLANG_VERSION, using fallback Neutron Clang 23"
            wget -q https://github.com/Neutron-Toolchains/clang-build-catalogue/releases/download/26052026/neutron-clang-26052026.tar.zst -O neutron.tar.zst
            mkdir -p clang && tar -xf neutron.tar.zst -C clang && rm neutron.tar.zst
            ;;
    esac
...
get_changelog() {
    if [ -f "changelog_manual.txt" ]; then
        cat "changelog_manual.txt" | head -10
        return
    fi
    
    if [ -f "CHANGELOG.md" ]; then
        head -10 "CHANGELOG.md" | sed 's/^# //' | sed 's/^## //'
        return
    fi
...
build_kernel() {
    ...
    case "${LTO:-none}" in
        "thin")
            ...
            setconfig enable LTO THINLTO LTO_CLANG ARCH_SUPPORTS_LTO_CLANG ARCH_SUPPORTS_THINLTO
            ;;
        "full")
            setconfig disable LTO_NONE THINLTO
            setconfig enable LTO LTO_CLANG ARCH_SUPPORTS_LTO_CLANG ARCH_SUPPORTS_THINLTO
            ;;
        *)
            setconfig enable LTO_NONE
            setconfig disable LTO THINLTO LTO_CLANG
            setconfig enable ARCH_SUPPORTS_LTO_CLANG ARCH_SUPPORTS_THINLTO
            ;;
    esac
...
post_build() {
    ...
    [ -n "$BUILD_TIME" ] && echo "BUILD_TIME=$BUILD_TIME" >> $GITHUB_ENV 2>/dev/null || true
...
Targets: kernel, defconfig, clean
Jobs: -j <number> or --jobs <number>

Env:
  LTO, KERNELSU, KSU_BRANCH, SELINUX_PERMISSIVE
  CLANG_VERSION, DEVICE, QCA_IS_MODULE, KCFLAGS

Changelog (priority):
  1. changelog_manual.txt
  2. CHANGELOG.md
...
if [ "$BUILD_TARGET" = "clean" ]; then
    pr_step "Cleaning..."
    rm -rf out toolchains *.zip changelog.txt 2>/dev/null
    make clean mrproper 2>/dev/null
    pr_info "Clean complete!"
    exit 0
fi

[[ "$JOB_FLAG" != "-j" && "$JOB_FLAG" != "--jobs" ]] && pr_err "Invalid job flag" && usage
[[ ! "$JOB_COUNT" =~ ^[0-9]+$ ]] && pr_err "Invalid job count" && usage
[[ "$BUILD_TARGET" != "kernel" && "$BUILD_TARGET" != "defconfig" ]] && pr_err "Invalid target" && usage
...
if [ "$BUILD_TARGET" = "defconfig" ]; then
    ...
    [ -f "out/.config" ] && cp out/.config out/build_config.txt && pr_info "Defconfig generated: out/.config" || { pr_err "Failed to generate defconfig"; exit 1; }
else
